...
FLUX tokens have infinite supply and their deflation is automatically adjusted by a "controlled on-chain burn" mechanism, which acts as a secondary function of money, to create a low volatility pool. Over 60% of all FLUX tokens have been utilized in this secondary function, creating on-chain velocity where FLUX is used as a form of "fuel" for this controlled burn. The introduction of ArbiFLUX and LOCK further refines this mechanism and enhances liquidity.
...
FLUX can only be generated by starting a validator with DAM tokens. FLUX is generated in a linear & predictable manner every 12 seconds. FLUX mint ratio scales linearly based on the amount of FLUX utilized in its secondary function (burning). With the addition of ArbiFLUX and LOCK, the ecosystem now features multi-layered minting and burning mechanisms that contribute to overall stability and efficiency, showcasing the secondary function of money for yield.

Uniswap and Balancer act as M0 money pools in our DeFi ecosystem. FLUX is constantly "re-cycled" through the liquidity pools via FLUX burning (as a secondary function for yield) & Minting cycles. With no DAM inflation and controlled FLUX deflation, and the unique liquidity-enhancing properties of LOCK, our liquidity pools offer high transaction throughput and robust market depth.
...
Our new **Datamine Market** and **Datamine Gems #GameFi** features further enhance this by providing real-time, interactive ways to engage with the ecosystem, driving transactional throughput and creating decentralized demand through gamified burning (as a secondary function for yield) and reward collection.
